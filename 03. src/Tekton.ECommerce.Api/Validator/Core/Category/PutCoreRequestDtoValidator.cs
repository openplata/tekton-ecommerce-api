...
{
    public class PutCoreRequestDtoValidator : AbstractValidator<PutCategoryRequestDto>
    {
        
        public PutCoreRequestDtoValidator()
        {
            RuleFor(x => x.CategoryId)
              .Cascade(CascadeMode.Stop)
              .NotNull().WithMessage(GlobalMessages.Required.Core.CategoryRequired.CategoryId)
              .Must(p => GlobalValidator<int>.IsNumeroPositivoEntero(p.Value)).WithMessage(GlobalMessages.Format.Core.CategoryFormat.CategoryId);
            ...
            RuleFor(x => x.IsActive)
              .Cascade(CascadeMode.Stop)
              .NotNull().WithMessage(GlobalMessages.Required.Core.CategoryRequired.IsActive)
              ;
